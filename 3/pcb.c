...
PcbPtr createnullPcb()
{
    ...
    new_process_Ptr->memory_requirement = 0;
    new_process_Ptr->memory_block = NULL;
...
{
    printf("%7d%7d%7d%7d%7d MB    %7d MB      ",
        (int) p->pid, p->arrival_time, p->priority,
            p->remaining_cpu_time, p->memory_requirement, p->memory_block->offset);
    switch (p->status) {
...
void printPcbHdr()
{  
    printf("    pid arrive  prior    cpu  memory_size  memory_offset  status\n");
}
